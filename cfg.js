...
if (!("window" in this)) // shells
  dumpln = print;
else if ("dump" in this) // gecko
  dumpln = function(s) { console.log(s + "\n"); };
else if ("console" in this) // safari (but also firebug)
  dumpln = function(s) { console.log(s); };
else if ("opera" in this && "postError" in opera) // opera -- suboptimial
  dumpln = window.opera.postError;
else
  dumpln = function(s) { };
...
function expandGrammarRecursion()
{
  ...
  gAccumulatedString = expandStrings(s0, s1, s2, s3, s4, RECURSION_TIMES);
}
...
function cfginit(filename, callback)
{
  ...
  // wtf is this needed to silence error console warnings? it's clearly .txt.
 //  req.overrideMimeType('text/plain');
  
  req.onreadystatechange = (function () {
    if (req.readyState == 4) {
      if (req.status == 200 || req.status == 0) {
        parseGrammar(req.responseText);
        callback();
      } else {
        dumpln("Error loading page: " + req.status);
      }
    }
  });
